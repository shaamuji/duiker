    ...
    if($result_rate !== false){
        foreach($result_rate as $i => $row){
            ...
            if($num_nights < $min_stay){
                ...
                $error = true;
                $res_hotel[$id_hotel][$id_room]['notice'] = $pms_texts['MIN_NIGHTS'].' : '.$min_stay;
            }
            if($adults+$children > $max_people){
                $error = true;
                $res_hotel[$id_hotel][$id_room]['notice'] = $pms_texts['MAX_PEOPLE'].' : '.$max_people;
                if(!$strict){
            ...
            if($adults+$children < $min_people){
                $error = true;
                $res_hotel[$id_hotel][$id_room]['notice'] = $pms_texts['MIN_PEOPLE'].' : '.$min_people;
                if(!$strict){
            ...
            if($adults > $max_adults){
                $error = true;
                $res_hotel[$id_hotel][$id_room]['notice'] = $pms_texts['MAX_ADULTS'].' : '.$max_adults;
                if(!$strict)
                    $num_adults = $max_adults;
            }
            if($children > $max_children){
                $error = true;
                $res_hotel[$id_hotel][$id_room]['notice'] = $pms_texts['MAX_CHILDREN'].' : '.$max_children;
                if(!$strict)
					...
					if(($price_sup > 0 || $extra_adults == 0) && ($num_people <= $people || $extra_price > 0)){
						...
						if(!isset($res_hotel[$id_hotel][$id_room]['total_nights']) || $res_hotel[$id_hotel][$id_room]['total_nights']+$nnights <= $num_nights){
							...
							if($num_nights < $res_hotel[$id_hotel][$id_room]['min_stay']){
								...
								$res_hotel[$id_hotel][$id_room]['notice'] = $pms_texts['MIN_NIGHTS'].' : '.$res_hotel[$id_hotel][$id_room]['min_stay'];
							}
						}
					}
                }
...
function getBookingSummary($room_id = null, $index = null, $amount = 0, $available = true)
{
        ...
        foreach($_POST['amount'] as $id_room => $values){
            foreach($values as $i => $value){
                if(isset($_POST['num_adults'][$id_room][$i]) && isset($_POST['num_children'][$id_room][$i])){
                    ...
                    $children = (isset($_POST['child_age'][$id_room][$i]) && is_array($_POST['child_age'][$id_room][$i])) ? array_filter_null($_POST['child_age'][$id_room][$i]) : array();
    ...
    if(!is_null($room_id)){
        ...
        if(isset($_POST['num_adults'][$room_id][$index]) && isset($_POST['num_children'][$room_id][$index])){
            ...
            $children = (isset($_POST['child_age'][$room_id][$index]) && is_array($_POST['child_age'][$room_id][$index])) ? array_filter_null($_POST['child_age'][$room_id][$index]) : array();
            $children = count($children);
    ...
    if($total > 0){
        ...
        <button name="book" id="btn-book_'.$hotel_id.'" class="btn btn-success btn-lg btn-block mt5"><b>'.$pms_texts['TOTAL'].' '.pms_formatPrice($total*PMS_CURRENCY_RATE).'</b> <i class="fas fa-fw fa-hand-point-right"></i> '.$pms_texts['BOOK'].'</button>';
