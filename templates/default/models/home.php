...
<section id="content" class="pt20 pb30">
    <div class="container">
        ...
        <div class="row mb10">
            <?php
            $result_hotel = $pms_db->query('SELECT * FROM pm_hotel WHERE lang = '.PMS_LANG_ID.' AND checked = 1 AND home = 1 ORDER BY `rank`');
    ...
    $result_destination = $pms_db->query('SELECT * FROM pm_destination WHERE lang = '.PMS_LANG_ID.' AND checked = 1 AND home = 1 ORDER BY `rank`');
    if($result_destination !== false){
        ...
        if($nb_destinations > 0){ ?>
            ...
            <div class="container">
                <div class="row mb10">
                    <h2 class="text-center mt5 mb10"><?php echo $pms_texts['TOP_DESTINATIONS']; ?></h2>
                    <?php
                    $destination_id = 0;
                    
                    $result_destination_file = $pms_db->prepare('SELECT * FROM pm_destination_file WHERE id_item = :destination_id AND checked = 1 AND lang = '.PMS_DEFAULT_LANG.' AND type = \'image\' AND file != \'\' ORDER BY `rank` LIMIT 1');
                    ...
                    foreach($result_destination as $i => $row){
                        ...
                        $destination_alias = DOCBASE.$pms_sys_pages['booking']['alias'].'/'.pms_text_format($row['alias']);
                        ...
                        <article class="col-sm-4 mb20" itemscope itemtype="http://schema.org/LodgingBusiness">
                            <a itemprop="url" href="<?php echo $destination_alias; ?>" class="moreLink">
                                <?php
                                if($result_destination_file->execute() !== false && $pms_db->last_row_count() == 1){
