            ...
            if($payment_type == 'paypal'){ ?>
                <div class="text-center">
                    ...
                    <form action="https://www.<?php if(PMS_PAYMENT_TEST_MODE == 1) echo 'sandbox.'; ?>paypal.com/cgi-bin/webscr" method="post">
                        ...
                        <input name="business" type="hidden" value="<?php echo (PMS_ENABLE_MULTI_VENDORS == 1 && $paypal_email != '') ? $paypal_email : PMS_PAYPAL_EMAIL; ?>">
                        <input name="item_name" type="hidden" value="<?php echo addslashes($_SESSION['tmp_book']['hotel'].' - '.gmstrftime(PMS_DATE_FORMAT, $_SESSION['tmp_book']['from_date']).' > '.gmstrftime(PMS_DATE_FORMAT, $_SESSION['tmp_book']['to_date']).' - '.$_SESSION['tmp_book']['nights'].' '.$pms_texts['NIGHTS'].' - '.($_SESSION['tmp_book']['adults']+$_SESSION['tmp_book']['children']).' '.$pms_texts['PERSONS']); ?>">
                ...
                <div class="text-center">
                    ...
                    <form action="https://<?php if(PMS_PAYMENT_TEST_MODE == 1) echo 'sandbox'; else echo 'www'; ?>.2checkout.com/checkout/purchase" method="post">
                        ...
                        <input type="hidden" name="li_0_name" value="<?php echo addslashes($_SESSION['tmp_book']['hotel'].' - '.gmstrftime(PMS_DATE_FORMAT, $_SESSION['tmp_book']['from_date']).' > '.gmstrftime(PMS_DATE_FORMAT, $_SESSION['tmp_book']['to_date']).' - '.$_SESSION['tmp_book']['nights'].' '.$pms_texts['NIGHTS'].' - '.($_SESSION['tmp_book']['adults']+$_SESSION['tmp_book']['children']).' '.$pms_texts['PERSONS']); ?>">
                ...
                require(SYSBASE.'includes/payments/razorpay/Razorpay.php');
                $api = new \Razorpay\Api\Api(PMS_RAZORPAY_KEY_ID, PMS_RAZORPAY_KEY_SECRET);
                $orderData = [
                    'receipt'         => $_SESSION['tmp_book']['id'],
                    'amount'          => round($payed_amount*100, 0),
                    'currency'        => 'INR',
                    'payment_capture' => 1
                ];
